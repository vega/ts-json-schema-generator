...
describe("deepMerge", () => {
    ...
    it("concatenates arrays", () => {
        ...
        expect(deepMerge([1, [2], 3], [1, [2], 3])).toEqual([1, [2], 3, 1, [2], 3]);
        expect(deepMerge([1, {foo: 'bar'}, 3], [1, {bar: 'foo'}, 3])).toEqual([1, {foo: 'bar'}, 3, 1, {bar: 'foo'}, 3]);
    });

    it("merges objects", () => {
        expect(deepMerge({foo: 'bar'}, {bar: 'foo'})).toEqual({foo: 'bar', bar: 'foo'});
        expect(deepMerge({foo: 'baz'}, {foo: 'bar'})).toEqual({foo: 'bar'});
        expect(deepMerge({flag: {type: 'boolean', enums: [true]}}, {flag: {type: 'boolean'}}))
            .toEqual({flag: {type: 'boolean', enums: [true]}});
        expect(deepMerge({flag: {type: 'boolean', enums: [true]}}, {flag: {type: 'boolean', enums: [false]}}))
